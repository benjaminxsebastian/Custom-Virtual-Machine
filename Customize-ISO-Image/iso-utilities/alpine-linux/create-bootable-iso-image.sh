...
then
    ...
    mkdir -p "$3"
    chmod +w "$2/$1/isolinux.bin"
    mkisofs -o "$destinationCustomIsoImagePath" -b  $1/isolinux.bin -c $1/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -J -R -V "Custom Image" -eltorito-alt-boot -eltorito-boot boot/grub/efi.img -no-emul-boot "$2"
    exitCode=$?
    if [ $exitCode != 0 ]
    then
        echo ""
